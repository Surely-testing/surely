...
export default function AdminPaymentsPage() {
  ...
  const loadPayments = async () => {
    setLoading(true);
    try {
      const result = await getAdminPayments(page, 50, filters);
      if (result.data) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-slate-600">Current Page Revenue</p>
              <p className="text-2xl font-bold text-slate-900">{formatCurrency(totalRevenue / 100)}</p>
            </div>
          </div>
        </div>
      </div>
              ...
              {loading ? (
                ...
                payments.map((payment) => (
                  <tr key={payment.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600">{payment.payment_method || 'Card'}</td>
