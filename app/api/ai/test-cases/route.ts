...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('🧪 Generating test cases for:', prompt)
    ...
    const normalizedTestCases = testCasesArray.map((tc: any, index: number) => ({
      // For display only - actual save will use different fields
    ...
    console.log(`✅ Generated ${normalizedTestCases.length} test cases`)
    ...
    console.error('❌ Test cases generation error:', error)
    return NextResponse.json({ 
      success: false, 
